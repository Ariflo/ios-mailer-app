                ...
                .background(
                    Capsule().fill(Color.gray.opacity(0.3).opacity(0.4))
                        .frame(width: frames[selectedIndex].width,
                               height: frames[selectedIndex].height, alignment: .topLeading)
...
struct CustomSegmentedPickerView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        CustomSegmentedPickerView(viewModel: TagIncomingLeadViewModel(addressableDataFetcher: AddressableDataFetcher()), tagOptions: [.fair])
    }
}
