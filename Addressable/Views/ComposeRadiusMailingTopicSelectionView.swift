//
//  ComposeRadiusMailingTopicSelectionView.swift
//  Addressable
//
...
struct ComposeRadiusMailingTopicSelectionView: View {
    ...
    init(viewModel: ComposeRadiusMailingViewModel, showAlert: @escaping () -> Void = { }) {
    ...
    var body: some View {
        VStack {
            if viewModel.loadingTopics {
                ...
                Spacer()
                EmptyListView(message: "No topics avaliable. Please reach out to your Addressable administrator or representative to create topics to select here and continue.")
                Spacer()
                    ...
                    VStack(alignment: .center, spacing: 25) {
                        VStack(spacing: 6) {
                            ...
                            isEditingTouchOne ? CheckView(
                                ...
                                toggle: { viewModel.shouldUpdateTouchOneTemplate = !viewModel.shouldUpdateTouchOneTemplate }
                        ...
                        if isEditingTouchOne {
                            ...
                            if !isTouchOnePreviewLoading {
                                ForEach(Array(viewModel.touchOneTemplateMergeVariables.keys), id: \.self) { mergeTagName in
                                    MergeVariableInput(viewModel: viewModel, touch: .touchOne, mergeTagName: mergeTagName)
                                }
                            }
                        }
                    }.padding(.top, 40)
                    ...
                    VStack(alignment: .center, spacing: 25) {
                        ...
                        if isEditingTouchTwo {
                            ...
                            if !isTouchTwoPreviewLoading {
                                ForEach(Array(viewModel.touchTwoTemplateMergeVariables.keys), id: \.self) { mergeTagName in
                                    MergeVariableInput(viewModel: viewModel, touch: .touchTwo, mergeTagName: mergeTagName)
                                }
                            }
                        }
                    }
            ...
            VStack {
                Button(action: {
                    if touch == .touchTwo || viewModel.shouldUpdateTouchOneTemplate {
                        if let templateID = touch == .touchOne ? viewModel.touchOneTemplate?.id : viewModel.touchTwoTemplate?.id {
                            viewModel.updateMessageTemplate(
                                for: touch,
                                id: templateID,
                                with: touch == .touchOne ? viewModel.touchOneBody : viewModel.touchTwoBody) { updatedTemplate in
                                guard updatedTemplate != nil else {
                                    showAlert()
                                    return
                                }
                                viewModel.getMessageTemplate(for: touch == .touchOne ? 1 : 2, with: updatedTemplate!.id) { template in
...
struct ComposeRadiusMailingTopicSelectionView_Previews: PreviewProvider {
    static var previews: some View {
        ComposeRadiusMailingTopicSelectionView(viewModel: ComposeRadiusMailingViewModel(selectedRadiusMailing: nil))
    }
}
