...
struct TargetCriteriaSliderView: View {
    ...
    @State var slider: CustomSlider = CustomSlider(start: 10, end: 100)
                ...
                VStack(spacing: 5) {
                    ...
                    case .livingArea:
                        Text("\(Int(slider.lowHandle.currentValue).roundedWithAbbreviations) - " +
                            ...
                            .font(Font.custom("Silka-Medium", size: 14))
                    case .lotSize:
                        Text("\(Int(slider.lowHandle.currentValue).roundedToNearestThousandWithAbbreviations) - \(Int(slider.highHandle.currentValue).roundedToNearestThousandWithAbbreviations)")
                            .font(Font.custom("Silka-Medium", size: 14))
                    default:
                        Text("\(String(Int(slider.lowHandle.currentValue))) - \(String(Int(slider.highHandle.currentValue)))")
                            .font(Font.custom("Silka-Medium", size: 14))
                    }
    ...
    private func toggleIncluded() {
        switch menuOption {
        case .homeValue:
            viewModel.dataTreeSearchCriteria.includeValue = !viewModel.dataTreeSearchCriteria.includeValue
            if viewModel.dataTreeSearchCriteria.includeValue {
                setCriteriaValues()
            }
        case .bedCount:
            viewModel.dataTreeSearchCriteria.includeBedCount = !viewModel.dataTreeSearchCriteria.includeBedCount
            if viewModel.dataTreeSearchCriteria.includeBedCount {
                setCriteriaValues()
            }
        case .bathCount:
            viewModel.dataTreeSearchCriteria.includeBathCount = !viewModel.dataTreeSearchCriteria.includeBathCount
            if viewModel.dataTreeSearchCriteria.includeBathCount {
                setCriteriaValues()
            }
        case .livingArea:
            viewModel.dataTreeSearchCriteria.includeBuildingArea = !viewModel.dataTreeSearchCriteria.includeBuildingArea
            if viewModel.dataTreeSearchCriteria.includeBuildingArea {
                setCriteriaValues()
            }
        case .yearBuilt:
            viewModel.dataTreeSearchCriteria.includeYearBuilt = !viewModel.dataTreeSearchCriteria.includeYearBuilt
            if viewModel.dataTreeSearchCriteria.includeYearBuilt {
                setCriteriaValues()
            }
        case .lotSize:
            viewModel.dataTreeSearchCriteria.includeLotSize = !viewModel.dataTreeSearchCriteria.includeLotSize
            if viewModel.dataTreeSearchCriteria.includeLotSize {
                setCriteriaValues()
            }
        case .percentEquity:
            viewModel.dataTreeSearchCriteria.includePercentEquity = !viewModel.dataTreeSearchCriteria.includePercentEquity
            if viewModel.dataTreeSearchCriteria.includePercentEquity {
                setCriteriaValues()
            }
        case .yearsOwned:
            viewModel.dataTreeSearchCriteria.includeYearsOwned = !viewModel.dataTreeSearchCriteria.includeYearsOwned
...
struct TargetCriteriaSliderView_Previews: PreviewProvider {
    static var previews: some View {
        TargetCriteriaSliderView(
            viewModel: ComposeRadiusMailingViewModel(selectedRadiusMailing: nil),
